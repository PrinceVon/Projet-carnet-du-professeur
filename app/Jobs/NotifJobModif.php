...
class NotifJobModif implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($evenement->rappel == 10 || $evenement->rappel == 30) {
            $message = "Vous avez un cours nommé \"{$evenement->titre}\" dans l'institution {$evenement->institution} à {$evenement->heure_debut}, dans {$evenement->rappel} minutes. Filière :  $evenement->filiere .";
        } elseif ($evenement->rappel == 60 || $evenement->rappel == 300) {
            $message = "Vous avez un cours nommé \"{$evenement->titre}\" dans l'institution {$evenement->institution} à {$evenement->heure_debut}, dans ".($evenement->rappel / 60).' heure(s). Filière :  $evenement->filiere .';
        } else {
